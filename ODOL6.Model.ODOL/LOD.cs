...
public class LOD : P3D_LOD, IComparable<LOD>
{
	...
	private static bool LooksLikePolygonStart(BinaryReaderEx input, long position)
	{
		uint? num = PeekUInt32(input, position);
		if (!num.HasValue || num.Value > 200000)
		{
			return false;
		}
		...
		try
		{
			...
			if (num.Value == 0)
			{
				return num2.Value == 0 && num3 == 0;
			}
			byte b = input.ReadByte();
			return num3 == 0 && b >= 3 && b <= 8;
		}
		catch
		{
			return false;
